...
class EventMediaService {
  ...
  static Future<String?> fetchCoverUrl(String eventId) async {
    final row = await _client
        .from('community_events')
        .select('cover_storage_path, cover_storage_provider')
        .eq('id', eventId)
        .maybeSingle();
    if (row == null) return null;
    return coverUrlForEvent(
      eventId: eventId,
      storagePath: row['cover_storage_path'] as String?,
      storageProvider: row['cover_storage_provider'] as String?,
    );
  }
