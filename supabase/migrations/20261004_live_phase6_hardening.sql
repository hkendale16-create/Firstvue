-- =============================================================================
-- FirstVue LIVE Phase 6: hardening (RPC-only presence writes, heat limits,
-- count helpers that avoid exposing other profile ids)
-- Additive + reversible.
-- =============================================================================

-- Presence writes must go through set_event_presence / clear_event_presence
-- (approved event + 4h TTL). Direct INSERT/UPDATE bypassed those guards.
...
-- Presence SELECT: own row only. Aggregates use event_here_now_count.
...
-- Hot count without listing everyone who reacted.
...
as $$
  ...
  from public.event_hot_reactions
  where event_id = p_event_id;
$$;

revoke all on function public.event_hot_count(uuid) from public;
grant execute on function public.event_hot_count(uuid) to authenticated, anon;

-- Hot SELECT: own row only (counts via event_hot_count).
...
-- Cap heat score batch size to reduce scrape/DoS via SECURITY DEFINER.
...
comment on function public.live_event_heat_scores(uuid[]) is
  'LIVE heat scores capped to 100 event ids. Null status = insufficient activity.';

comment on function public.event_hot_count(uuid) is
  'Hot reaction count for an event without exposing reactor profile ids.';
