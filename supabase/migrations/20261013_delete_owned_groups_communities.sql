-- Owner/leader permanent deletion for Groups (communities) and
-- Communities (community_hubs). Mirrors delete_owned_business.
...
begin
  ...
  -- Remove group-scoped news posts (media/sparks cascade where configured).
...
begin
  ...
  -- Linked Groups are unlinked (hub_id ON DELETE SET NULL). Do not wipe Groups.
  -- Clear creation-request pointer explicitly for clarity.
